...
class App:
    ...
    def login(self):
        ...
        name = output.split(',')[1]
        name = name.split('\\')[0]
        if name in ['unknown_person', 'no_preson_found']:
...
#  1.0: This refers to the starting position of the text. In Tkinter, text positions
#  are given in the format line.column. So, 1.0 means the first line, at the first character (the very beginning of the text).

#  'end-1c':
    # 'end': Refers to the end of the text in the Text widget.
    # -1c: Means "minus one character." Tkinter's Text widget adds a newline (\n) 
    # character at the end of the content, so using 'end-1c' removes that extra newline.
